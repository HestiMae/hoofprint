...
public class HoofprintScreen extends Screen {
	...
	private void renderPlayer(DrawContext context, PlayerSummary player, UUID uuid) {
		...
		context.getMatrices().translate((float) clampedX, (float) clampedY);
		int tint = !player.online() ? 77 : mouseOver ? 204 : 255;
		int argb = ColorHelper.getArgb(255, (friend ? 0 : 255) * tint / 255, (inDim ? 255 : 204) * tint / 255, (friend ? 76 : 255) * tint / 255);
		if (Math.abs(playerScreenX - clampedX) > width || Math.abs(playerScreenY - clampedY) > height) {
			context.getMatrices().translate(-2, -2);
			context.drawTexture(RenderPipelines.GUI_TEXTURED, Identifier.tryParse("textures/map/decorations/player_off_limits.png"), 0, 0, 2, 2, 4, 4, 4, 4, 8, 8, argb);
		} else if (clipped) {
			context.getMatrices().translate(-3, -3);
			context.drawTexture(RenderPipelines.GUI_TEXTURED, Identifier.tryParse("textures/map/decorations/player_off_map.png"), 0, 0, 1, 1, 6, 6, 6, 6, 8, 8, argb);
		} else {
			...
			context.drawTexture(RenderPipelines.GUI_TEXTURED, Identifier.tryParse("textures/map/decorations/player.png"), 0, 0, 2, 0, 5, 7, 5, 7, 8, 8, argb);
		}
		context.getMatrices().popMatrix();
	}
	...
	private void renderOffscreenMapArrow(DrawContext context) {
		...
		if (mapOffscreen) {
			...
			context.getMatrices().rotate((float) (Math.PI + angle));
			context.getMatrices().translate(-3.5F, -4F);
			context.drawTexture(RenderPipelines.GUI_TEXTURED, Identifier.tryParse("textures/map/decorations/target_point.png"), 0, 0, 1, 0, 7, 8, 7, 8, 8, 8);
			context.getMatrices().popMatrix();
		}
	}
