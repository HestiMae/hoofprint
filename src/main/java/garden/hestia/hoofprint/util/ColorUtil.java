...
public class ColorUtil {
	...
	public static final Map<Predicate<Block>, Function<Biome, Integer>> BLOCK_COLOR_PROVIDERS = Map.of(
		BlockConstants.FOLIAGE_BLOCKS::contains, biome -> ColorUtil.tint(ColorConstants.FOLIAGE_TEXTURE_COLOR, biome.getFoliageColor()),
		BlockConstants.GRASS_BLOCKS::contains, biome -> ColorUtil.tint(ColorConstants.GRASS_TEXTURE_COLOR, biome.getFoliageColor()),
		BlockConstants.GRASS_BLOCK_BLOCKS::contains, biome -> ColorUtil.tint(ColorConstants.GRASS_BLOCK_TEXTURE_COLOR, biome.getFoliageColor())
	);
	...
	public static int getBlendedBiomeColor(Biome[][] biome, Function<Biome, Integer> getter, int x, int z, int radius) {
		if (radius == 0) return getter.apply(biome[x][z]);
		long r = 0;
		long g = 0;
		long b = 0;
		int num = 0;
		for (int i = x - radius; i < x + radius; i++) {
			for (int j = z - radius; j < z + radius; j++) {
				if (i < 0 || j < 0 || i >= biome.length || j >= biome[i].length || biome[i][j] == null || (x - i) * (x - i) + (z - j) * (z - j) > radius * radius) continue;
				int c = getter.apply(biome[i][j]);
				r += (c & 0xFF0000) >> 16;
				g += (c & 0xFF00) >> 8;
				b += c & 0xFF;
				num++;
	...
	public static Function<Biome, Integer> getBiomeColorProvider(Block block) {
